...
- **状态面板**：`/admin` 实时查看上游健康状态、累计转发数、在途连接与最近请求分发记录
...
访问 `http://<listen>/admin` 可查看：

- 每个上游的实时健康状态（绿/红）、权重、优先级、在途连接数、累计转发次数
- 最近 200 条请求记录（时间 + 路径 + 实际转发到了哪个上游）
- 页面每 3 秒自动刷新；数据接口为 `/admin/api`（JSON）

```yaml
# config.yaml 可选配置
admin_path: "/admin"          # 面板路径（默认 /admin）
admin_token: "your-token"     # 访问面板的 token（留空则不鉴权，公网部署强烈建议配置）
```
